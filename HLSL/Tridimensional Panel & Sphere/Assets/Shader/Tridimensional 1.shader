    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed3 formaFinal = fixed3(0.0,0.0,0.0);
                ...
                {
                    float index = float(j) * PI * 2.0 / float(cantidad);

                    fixed2 movimiento = fixed2(sin(_time+index), cos(_time + index));
                    uv2 = frac(i.uv * (float(j) + 1.0)+ movimiento);
                    float2 position = fixed2(0.5 * fix - 0.5, 0.5);
                    _point = position - uv2 ;
                    float radio = length(_point);

                    fixed3 color1 = fixed3(1.0,1.0,0.0);
                    fixed3 color2 = fixed3(1.0,0.0,0.0);

                    float indice_color = (float(j) + 1.0) / float(cantidad);
                    fixed3 colorFinal = lerp(color1 , color2 ,indice_color);

                    formaFinal += smoothstep(0.95,0.99,1.0-radio)* colorFinal;
                    uvAnterior = uv2;
